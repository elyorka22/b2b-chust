...
export default function SuperAdminDashboard() {
  ...
  return (
    <div>
      ...
      {activeTab === 'orders' && (
        <div>
          <h2 className="text-2xl font-semibold mb-4">Barcha buyurtmalar</h2>
          {loading ? (
            <div className="text-center py-12">Yuklanmoqda...</div>
          ) : (
      ...
      {activeTab === 'users' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold">Foydalanuvchilarni boshqarish</h2>
            <button
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        alert(data.error || 'Foydalanuvchi yaratishda xatolik');
      }
    } catch (error) {
